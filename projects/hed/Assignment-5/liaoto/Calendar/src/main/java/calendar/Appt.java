...
public class Appt{
	...
	public void setValid() {
		...
		if (startMonth < 1 || startMonth > 12)
			this.valid = false;
		else if (startHour < 0 || startHour > 23) //bug found start hour is more than instead of less than 
			this.valid = false;
			...
			if (startDay < 1 || startDay > NumDaysInMonth)
				this.valid = false; //bug found this should be false not true
			else
				this.valid = true;
		}
	}
    ...
    private String represntationApp(){
        ...
        if (printableHour == 0)
        {
            printableHour = 12; //Bug found printable hour supposed to be 12 not 0
        }
