...
public class Appt{
	...
	public void setValid() {
		...
		if (startMonth < 1 || startMonth > 12)
			this.valid = false;
		else if (startHour < 0 || startHour > 23) //Found BUG the start > 23 was to <
			this.valid = false;
			...
			if (startDay < 1 || startDay > NumDaysInMonth)
				this.valid = false; //Found BUG supposed to be false if the if statement is passed
			else
				this.valid = true;
		}
	}
    ...
    private String represntationApp(){
        ...
        if (printableHour == 0)
        {
            printableHour = 12; //found BUG the hour should be 12 and not 0
        }
